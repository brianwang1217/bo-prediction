...
# initialization
def bo_train(epochs=500):
	movie_data = load_data("movie_data.csv")
	budget, domestic, worldwide = get_parameters(movie_data)

	# For now, we'll do simple linear regression to predict domestic success, given budget.
	x_train, x_test, y_train, y_test = split_data(budget, domestic) # Can replace domestic with worldwide.
	...
	error_list.append(error(x_train, y_train, m, b))

	for i in range(epochs):
		m, b = lr_gradient_step(x_train, y_train, m, b)
		error_list.append(error(x_train, y_train, m, b))

	test_error = error(x_test, y_test, m, b)

	# Print final error, as well as test_error.
	print(error_list[-1], test_error)	

	# Graph time!
	graph_error(error_list)
	graph_model(x_train, y_train, m, b, epochs)
...
# Gradient steps
def lr_gradient_step(x, y, m_current, b_current, learning_rate = 0.00001):
	n = float(len(x))
	b_gradient = 0
	m_gradient = 0
...
def graph_model(x, y, m, b, epochs):
	plt.scatter(x, y, s=2)
